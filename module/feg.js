...
Hooks.once("ready", async function() {
  ...
  Hooks.on("hotbarDrop", (bar, data, slot) => createFEGMacro(data, slot));
});
 ...
 * @returns {Promise}
 */
async function createFEGMacro(data, slot) {
  if (data.type !== "Item") return;
  if (!("data" in data)) return ui.notifications.warn("You can only create macro buttons for owned Items");
  const item = data.data;

  // Create the macro command
  const command = `game.fegsystem.rollItemMacro("${item.name}");`;
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command: command,
      flags: { "fegsystem.itemMacro": true }
    });
  }
